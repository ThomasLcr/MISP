...
# [1] Argument check
# Ensure the user provides a valid number of instances (>2)
if [ -z "$1" ] || ! [[ "$1" =~ ^[0-9]+$ ]] || [ "$1" -le 2 ]; then
  echo "Usage: $0 <number_of_db_and_misp_containers> (must be >2)"
  exit 1
fi
...
# Install docker-compose if not present, and set compose command
...
# Prompt user to enter admin API keys for each instance
echo "Please log in to each instance to retrieve the admin API keys"
for i in $(seq 1 "$NUM_INSTANCES"); do
  HOSTS[$i]="localhost:808$i"
  echo -n "Admin API key for instance misp_$i (${HOSTS[$i]}): "
  read -r auth_key
  AUTHS[$i]="$auth_key"
...
# Example synchronization schema for demonstration
declare -A EXAMPLE_SCHEMA=(
  [1]="2 5"
  [2]="1 3"
  [3]="2 4"
  [4]="3"
...
echo -e "\n[+] EXAMPLE_SCHEMA content"
for key in "${!EXAMPLE_SCHEMA[@]}"; do
  echo "Instance $key ↔️  ${EXAMPLE_SCHEMA[$key]}"
done

# Prompt user to define synchronization schema between instances
echo -e "\n[?] Define the synchronization schema between instances"
for i in $(seq 1 "$NUM_INSTANCES"); do
  echo -n "→ Which instances are connected to instance $i (numbers separated by spaces, e.g.: 2 5)? "
  read -r connexions
  CONNECTION_SCHEMA[$i]="$connexions"
done

echo -e "\n[+] CONNECTION_SCHEMA content"
for key in "${!CONNECTION_SCHEMA[@]}"; do
...
for i in "${!HOSTS[@]}"; do
  ...
  echo "export AUTH_$i=\"${AUTHS[$i]}\"" >> "$ENV_FILE"
